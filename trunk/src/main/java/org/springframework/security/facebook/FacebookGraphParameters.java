package org.springframework.security.facebook;

/**
 * Created by IntelliJ IDEA.
 * User: porter
 * Date: Jul 28, 2010
 * Time: 8:37:49 PM
 * To change this template use File | Settings | File Templates.
 */
public class FacebookGraphParameters {
    public static String FACEBOOK_GRAPH_PARAMETER_CLIENT_ID = "client_id";
    public static String FACEBOOK_GRAPH_PARAMETER_CLIENT_SECRET = "client_secret";
    public static String FACEBOOK_GRAPH_PARAMETER_REDIRCT_URL = "redirect_uri";
    public static String FACEBOOK_GRAPH_PARAMETER_CODE = "code";
    public static final String FACEBOOK_GRAPH_PARAMETER_SCOPE = "scope";
    public static final String FACEBOOK_GRAPH_OAUTH_URL = "https://graph.facebook.com/oauth/authorize";
    public static final String FACEBOOK_GRAPH_USER_INFO_BASE = "https://graph.facebook.com/";
    public static final String FACEBOOK_GRAPH_PARAMETER_ACCESS_TOKEN = "access_token";

}
